...
用于图像数据的实时数据增强的（相当）基本的工具集。
Can easily be extended to include new transformations,
new preprocessing methods, etc...
可以很容易地扩展到(包括)新的变换，新的预处理方法等。
"""
...
if pil_image is not None:
    ...
    # 这些方法只在3.4.0（2016）版本中引入。
    if hasattr(pil_image, 'HAMMING'):
        _PIL_INTERPOLATION_METHODS['hamming'] = pil_image.HAMMING
    if hasattr(pil_image, 'BOX'):
        _PIL_INTERPOLATION_METHODS['box'] = pil_image.BOX
    # This method is new in version 1.1.3 (2013).
    # 这个方法只在1.1.2（2013）版本中引入。
...
def random_shift(x, wrg, hrg, row_axis=1, col_axis=2, channel_axis=0,
    ...
    参数
        ...
        wrg: 宽度偏移范围，作为宽度的浮动部分。
        hrg: Height shift range, as a float fraction of the height.
        hrg: 高度移动范围，作为高度的浮动部分。
        row_axis: Index of axis for rows in the input tensor.
        row_axis: 输入张量中的行的轴索引。
        col_axis: Index of axis for columns in the input tensor.
        col_axis: 输入张量中的列的轴索引。
        channel_axis: Index of axis for channels in the input tensor.
        channel_axis: 输入张量中的通道的轴的索引。
        fill_mode: Points outside the boundaries of the input
            are filled according to the given mode
    ...
    transform_matrix = translation_matrix  # no need to do offset # 无需做偏移
    x = apply_transform(x, transform_matrix, channel_axis, fill_mode, cval)
    return x
...
def random_zoom(x, zoom_range, row_axis=1, col_axis=2, channel_axis=0,
    ...
    执行数字图像张量的随机空间缩放（缩小放大）。
    # Arguments
    参数
        x: Input tensor. Must be 3D.
        x: 输入张量。必须是3维
        zoom_range: Tuple of floats; zoom range for width and height.
        zoom_range: 浮动的元组；缩放范围的宽度和高度。
        row_axis: Index of axis for rows in the input tensor.
        row_axis: 输入张量中的行的轴索引。
        col_axis: Index of axis for columns in the input tensor.
        col_axis: 输入张量中列的轴索引。
        channel_axis: Index of axis for channels in the input tensor.
        channel_axis: 输入张量中通道的轴索引。
        fill_mode: Points outside the boundaries of the input
            are filled according to the given mode
            (one of `{'constant', 'nearest', 'reflect', 'wrap'}`).
        fill_mode: 输入的边界之外的点根据给定的模式（{常数”、“最近”、“反射”、“包装”}中的一个填充。
...
def array_to_img(x, data_format=None, scale=True):
    ...
    将3维 Numpy数组转换为PIL（Python Imaging Library ）图像实例。
    # Arguments
    参数
        x: Input Numpy array.
        x: 输入数字数组。
    ...
    补充
        ...
        ImportError: 如果PIL不可行
        ValueError: if invalid `x` or `data_format` is passed.
        ValueError: 如果无效的`x` 或 `data_format` 被传递。
    """
    ...
    # 原始的Numpy数组x具有格式（高度、宽度、通道）
    # or (channel, height, width)
    # 或者(通道, 高度、宽度)
    # but target PIL image has format (width, height, channel)
    # 但是目标PIL图像具有格式（宽度、高度、通道）
...
def img_to_array(img, data_format=None):
    ...
    补充
        ...
        ValueError: 如果无效的“IMG”或“data_format”被传递。
    """
    if data_format is None:
        data_format = K.image_data_format()
    if data_format not in {'channels_first', 'channels_last'}:
...
def load_img(path, grayscale=False, target_size=None,
    ...
    参数
        ...
        grayscale: bool型, 是否将图像加载为灰度级。
        target_size: Either `None` (default to original size)
            or tuple of ints `(img_height, img_width)`.
        target_size: 要么“None”（默认为原始大小），要么是int的元组“（img_height, img_width）”。
...
class ImageDataGenerator(object):
    ...
    参数
        ...
        featurewise_center:在数据集上设置输入平均为0。
        samplewise_center: set each sample mean to 0.
        samplewise_center: 将每个样本均值设为0。
        featurewise_std_normalization: divide inputs by std of the dataset.
        featurewise_std_normalization: 通过数据集的std划分输入。
        samplewise_std_normalization: divide each input by its std.
        samplewise_std_normalization: 用std划分每个输入。
        zca_whitening: apply ZCA whitening.
        zca_whitening: 应用ZCA白化
        # https://blog.csdn.net/hjimce/article/details/50864602
        zca_epsilon: epsilon for ZCA whitening. Default is 1e-6.
        zca_epsilon: ZCA编号参数epsilon。默认1e-6
        rotation_range: degrees (0 to 180).
        rotation_range: 角度 (0 至 180).
        width_shift_range: fraction of total width, if < 1, or pixels if >= 1.
        width_shift_range:总宽度的分数，如果＜1，或像素如果＞=1。
        height_shift_range: fraction of total height, if < 1, or pixels if >= 1.
        height_shift_range: 总高度的分数，如果＜1，或像素如果＞=1。
        shear_range: shear intensity (shear angle in degrees).
        shear_range: 剪切强度（剪切角为度）。
        zoom_range: amount of zoom. if scalar z, zoom will be randomly picked
            in the range [1-z, 1+z]. A sequence of two can be passed instead
            to select this range.
        zoom_range:缩放量。如果标量Z，缩放将随机选择在范围[1-Z，1 +Z]。可以通过两个序列来选择这个范围。
        channel_shift_range: shift range for each channel.
        channel_shift_range: 每个通道的移位范围。
        ...
        fill_mode:边界外的点按照给定的模式（'constant', 'nearest', 'reflect' or 'wrap'）填充。默认是“nearest”。
                根据给定的模式填充输入边界之外的点：
                 'constant': kkkkkkkk|abcd|kkkkkkkk (cval=k)
                'nearest':  aaaaaaaa|abcd|dddddddd
                'reflect':  abcddcba|abcd|dcbaabcd
        ...
        cval: 当填充模式为“constant”时，用于边界外的点的值。默认值为0。
        horizontal_flip: whether to randomly flip images horizontally.
        horizontal_flip: 是否随机地水平翻转图像。
        vertical_flip: whether to randomly flip images vertically.
        vertical_flip: 是否随机地垂直翻转图像。
        rescale: rescaling factor. If None or 0, no rescaling is applied,
            otherwise we multiply the data by the value provided. This is
            applied after the `preprocessing_function` (if any provided)
            but before any other transformation.
        rescale:尺度因子。如果没有或0，没有尺度缩放应用，否则我们乘以数据提供的值。这是在“预处理函数”（如果有的话）
        之后，但在任何其他转换之前应用的。
        ...
        preprocessing_function:将在每个输入中隐含的函数。
            该函数将在对其进行任何其他修改之前运行。
            这个函数应该有一个参数：
            一个图像（秩为3的Numpy张量），输出一个相同形状的Numpy张量。
        data_format: 'channels_first' or 'channels_last'. In 'channels_first' mode, the channels dimension
            (the depth) is at index 1, in 'channels_last' mode it is at index 3.
            It defaults to the `image_data_format` value found in your
            Keras config file at `~/.keras/keras.json`.
            If you never set it, then it will be "channels_last".
        data_format:  'channels_first' or 'channels_last'.在'channels_first' 模式，通道维度（深度）是索引1，在
            'channels_last' 模式，通道维度是3.默认是``image_data_format` 值，在Keras配置文件`~/.keras/keras.json`。
            如果不设置，默认为"channels_last".
    """
